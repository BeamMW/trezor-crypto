...
void calculator_aggregator_proceed_rec(_calculator_aggregator_t *ag, uint32_t i_pos, uint32_t i_cycle, const scalar_t *k, uint32_t j)
{
  if (ag->x[j])
  {
    ...
    calculator_aggregator_proceed(ag, i_pos, i_cycle - 1, k); // in batch mode all inverses are already multiplied
}
...
void calculator_aggregator_proceed(_calculator_aggregator_t *ag, uint32_t i_pos, uint32_t i_cycle, const scalar_t *k)
{
  ...
  else
  {
    if (ag->calc)
    {
      multi_mac_casual_init(&ag->mm->casual[ag->mm->n_casual++], &ag->calc->gen[ag->j][i_pos], k);
    }
    else
    {
      // if (ag->batch_ctx) {...} else
      calculator_modifier_expanded_set(ag->mod, &ag->mm->k_prepared[ag->mm->n_prepared], k, i_pos, ag->j);
      ag->mm->prepared[ag->mm->n_prepared++] = (multi_mac_prepared_t *)get_generator_ipp(i_pos, ag->j, 0);
    }
  }
}
...
void inner_product_calculator_extract_LR(inner_product_calculator_t *calc, int j)
{
  ...
  calc->mm.prepared[calc->mm.n_prepared++] = (multi_mac_prepared_t *)get_generator_dot_ipp();
  ...
  for (int jSrc = 0; jSrc < 2; jSrc++)
  {
    ...
    for (uint32_t i = 0; i < calc->n; i++)
    {
      ...
      calculator_aggregator_init(&aggr, &calc->mm, &calc->cs.x[0], &calc->cs.x[1], &calc->mod, jSrc, INNER_PRODUCT_N_CYCLES - calc->i_cycle);

      if (calc->i_cycle > INNER_PRODUCT_I_CYCLE_0)
        aggr.calc = calc;
...
void inner_product_calculator_condense(inner_product_calculator_t *calc)
{
  ...
  for (int j = 0; j < 2; j++)
    for (uint32_t i = 0; i < calc->n; i++)
    {
      ...
      scalar_t r;
      scalar_mul(&calc->val[j][i], &calc->src[j][i], &calc->cs.x[j].val[calc->i_cycle]);
      scalar_mul(&r, &calc->src[j][calc->n + i], &calc->cs.x[!j].val[calc->i_cycle]);
  ...
  for (int j = 0; j < 2; j++)
    for (uint32_t i = 0; i < calc->n; i++)
    {
      ...
      _calculator_aggregator_t aggr;
      calculator_aggregator_init(&aggr, &calc->mm, &calc->cs.x[0], &calc->cs.x[1], &calc->mod,
                                 j, INNER_PRODUCT_N_CYCLES - calc->i_cycle - 1);
                          ...
                          const scalar_t *a, const scalar_t *b, inner_product_modifier_t *mod)
{
  ...
  if (ab)
  {
    for (uint32_t j = 0; j < 2; j++)
    {
      for (uint32_t i = 0; i < INNER_PRODUCT_N_DIM; i++, calc.mm.n_prepared++)
      {
        ...
        calculator_modifier_expanded_set(&calc.mod, &calc.mm.k_prepared[calc.mm.n_prepared], &calc.src[j][i], i, j);
      }
    }
  ...
  for (calc.i_cycle = 0; calc.i_cycle < INNER_PRODUCT_N_CYCLES; calc.i_cycle++)
  {
    calc.n = INNER_PRODUCT_N_DIM >> (calc.i_cycle + 1);

    scalar_create_nnz(oracle, &calc.cs.x[0].val[calc.i_cycle]);
    scalar_inverse(&calc.cs.x[1].val[calc.i_cycle], &calc.cs.x[0].val[calc.i_cycle]);
    ...
    if (!calc.i_cycle)
      for (int j = 0; j < 2; j++)
        calc.src[j] = calc.val[j];
  }

  for (int i = 0; i < 2; i++)
    in->condensed[i] = calc.val[i][0];
